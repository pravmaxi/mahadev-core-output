...
INPUT_SHEET = "input5"  # --> Change to "input1" for trade-focused sheet or F&O
OUTPUT_SHEET = "sroutput_multitf" if MULTI_TF_MODE else f"niftysroutput_{SELECTED_TF}"
JSON_FILE = f"niftysup&res_{SELECTED_TF}.json"
...
def read_symbols(sheet):
    ...
    for s in raw_symbols:
        # Keep index symbols (starting with ^) and exchange-prefixed symbols as-is
        if s.startswith("^") or ":" in s:
            processed.append(s)
        else:
            processed.append(f"{s}.NS")   # Append .NS for regular NSE stocks
    return processed
